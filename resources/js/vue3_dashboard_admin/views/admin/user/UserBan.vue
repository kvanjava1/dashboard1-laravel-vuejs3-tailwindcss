                    ...
                    <ContentBoxBody>
                        ...
                        <div v-else class="space-y-4">
                            <div
                                ...
                                :key="history.id"
                                :class="[
                                    'relative p-5 rounded-xl border-l-4 shadow-sm transition-all hover:shadow-md',
                                    history.action === 'ban'
                            ...
                            >
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 class="font-semibold text-slate-800 capitalize">{{ history.action }} Action</h4>
                                            <p class="text-sm text-slate-500">{{ formatDate(history.created_at) }}</p>
                                        </div>
                                    </div>
                                    <span
                                        :class="[
                                    ...
                                    >
                                        {{ history.action === 'ban' ? 'Banned' : 'Unbanned' }}
                                    </span>
                                </div>

                                <div class="ml-14 space-y-2">
                                    <p class="text-slate-700 font-medium">{{ history.reason }}</p>

                                    <div v-if="history.action === 'ban'" class="flex items-center gap-2 text-sm">
                                        <span class="material-symbols-outlined text-slate-500 text-sm">schedule</span>
                                        <span class="text-slate-600">
                                            <span v-if="history.is_forever" class="font-medium text-red-600">Permanent Ban</span>
                                            <span v-else-if="history.banned_until" class="font-medium">Until {{ formatDate(history.banned_until) }}</span>
                                        </span>
                                    </div>

                                    <div class="flex items-center gap-2 text-sm text-slate-500">
                                        <span class="material-symbols-outlined text-xs">person</span>
                                        <span>By {{ history.performed_by_name || 'System' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <ContentBoxHeader>
                        <template #title>
                            <div class="flex items-center gap-2">
                                ...
                                <ContentBoxTitle title="Ban User" />
                            </div>
                        </template>
                    </ContentBoxHeader>
                    <ContentBoxBody>
